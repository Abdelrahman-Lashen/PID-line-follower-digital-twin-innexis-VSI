...
class Visualizer:
    def __init__(self, args):
        ...
        print("=== Visualizer started ===", flush=True)
    ...
    def mainThread(self):
        ...
        try:
            ...
            while vsiCommonPythonApi.getSimulationTimeInNs() < self.totalSimulationTime:
                try:
                    ...
                    if len(self.times) % 50 == 0:
                        print(f"  t={self.t:5.1f}s | robot=({rx:.2f},{ry:.3f}) | error={lat_err:.3f}m | omega={omega:.3f}", flush=True)
                except:
                    pass
                self.updateInternalVariables()
                if vsiCommonPythonApi.isStopRequested():
            ...
            if str(e) == "stopRequested":
                ...
                print(f"Visualizer error: {str(e)}", flush=True)
        except:
            vsiCommonPythonApi.advanceSimulation(self.simulationStep + 1)
        self.save_results()
    ...
    def save_results(self):
        ...
        print(f"\n=== KPIs ===", flush=True)
        print(f"  Overshoot:          {overshoot:.4f} m", flush=True)
        print(f"  Settling time:      {settling_time:.4f} s", flush=True)
        print(f"  Steady-state error: {ss_error:.4f} m", flush=True)
        ...
        print("=== Plots saved to outputs/ ===", flush=True)
