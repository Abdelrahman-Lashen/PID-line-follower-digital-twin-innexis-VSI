...
class Controller:
    def __init__(self, args):
        ...
        print("    PID: Kp=2.0  Ki=0.1  Kd=0.5  alpha=0.5", flush=True)
    ...
    def mainThread(self):
        ...
        try:
            ...
            while vsiCommonPythonApi.getSimulationTimeInNs() < self.totalSimulationTime:
                ...
                self._step += 1
                if self._step % 50 == 0:
                    e_lat = abs((self.rx-self.px)*self.ty - (self.ry-self.py)*self.tx)
                    print(f"  t={self._step*0.01:5.1f}s | rx={self.rx:6.2f}m | ry={self.ry:7.3f}m | omega={self.omega_cmd:6.3f} | e_lat={e_lat:.3f}m", flush=True)
            ...
            if str(e) == "stopRequested":
                ...
                print(f"Controller error: {str(e)}", flush=True)
        except:
            vsiCommonPythonApi.advanceSimulation(self.simulationStep + 1)
